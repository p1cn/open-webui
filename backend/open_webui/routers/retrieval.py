...
def search_web(request: Request, engine: str, query: str) -> list[SearchResult]:
    ...
    if engine == "searxng":
        ...
        if (
            request.app.state.config.TAVILY_API_KEY
            and request.app.state.config.EXA_API_KEY
        ):
            ...
            raise Exception(
                "No TAVILY_API_KEY or EXA_API_KEY found in environment variables"
            )
...
async def process_web_search(
    ...
    try:
        ...
        if request.app.state.config.RAG_WEB_SEARCH_ENGINE in (
            "exa",
            "tavily",
            "jina",
            "tavily+exa",
