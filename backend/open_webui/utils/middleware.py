...
async def process_chat_payload(request, form_data, metadata, user, model):
    ...
    log.info(f"sources: {json.dumps(sources)}")
    if len(sources) > 0:
        context_string = ""
            ...
            if "document" in source:
                for doc_idx, doc_context in enumerate(source["document"]):
                    ...
                    if model["architecture"]["tokenizer"] == "DeepSeek":
                        if source_type == "web_search_results":
        ...
        # DeepSeek tool prompt
            ...
            log.debug(f'DeepSeek tool prompt: {request.app.state.config.RAG_DEEPSEEK_TEMPLATE.replace("{{SEARCH_RESULTS}}", webpage_context_string).replace("{{FILE_CONTEXT}", file_context_string).replace("{{CUR_DATE}}", time.strftime("%Y-%m-%d %H:%M:%S")).replace("{{QUESTION}}", prompt)}')
            form_data["messages"] = add_or_update_system_message(
                    request.app.state.config.RAG_DEEPSEEK_TEMPLATE.
                    replace("{{SEARCH_RESULTS}}", webpage_context_string).
                    replace("{{FILE_CONTEXT}", file_context_string).
                    replace("{{CUR_DATE}}", time.strftime("%Y-%m-%d")).
